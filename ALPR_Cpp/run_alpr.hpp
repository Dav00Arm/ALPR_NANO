...
void alpr(std::vector<QLabel*> labels, std::vector<std::string> cam_names, std::vector<std::string> capture_source){
    ...
    for(int cam_id=0; cam_id < capture_source.size(); ++cam_id){
        ...
        // alpr_multithreading(cam_id, bboxes[cam_id], labels[cam_id], label_size, cam_names[cam_id], std::ref(cam));
        camera_thread = new std::thread(alpr_multithreading, cam_id, bboxes[cam_id], labels[cam_id], label_size, cam_names[cam_id], std::ref(cam));
        camera_threads.push_back(camera_thread);
    }

    for(int thread_id=0; thread_id < camera_threads.size(); ++thread_id){
        camera_threads[thread_id]->join();
    }
    ...
    //                 car_colors_dict = color_classifier(car_images, car_ind_dict, 0, 0, 0, 0);

    //                 for(auto spot_dict_pair: current_spot_dict){
    //                     auto key = spot_dict_pair.first;
    //                     std::cout<<"spot dict pair"<<std::endl;
    //                     std::cout<<spot_dict_pair.first<<" "<< spot_dict_pair.second<<std::endl;
    ...
    //                         std::cout<<"status size: "<<status[spots_i].size()<<std::endl;
    ...
    //                                 cv::imwrite("images/img_" + color + "_" + label + ".jpg", car_images[car_ind_dict[one_spot_dict.first]]);
